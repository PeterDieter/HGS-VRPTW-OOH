...
void Genetic::run(int maxIterNonProd, int timeLimit)
{
	...
	for (int nbIter = 0; nbIterNonProd <= maxIterNonProd && !params->isTimeLimitExceeded(); nbIter++)
	{
		...
		localSearch->run(offspring, params->penaltyVehicleCapacity, params->penaltyTimeWarp, params->penaltyParcelCapacity, true);
		...
		if (!offspring->isFeasible && params->rng() % 100 < (unsigned int) params->config.repairProbability)
		{
			...
			localSearch->run(offspring, params->penaltyVehicleCapacity * 10., params->penaltyTimeWarp * 10., params->penaltyParcelCapacity * 10., true);
...
void Genetic::doOXcrossover(Individual* result, std::pair<const Individual*, const Individual*> parents, int start, int end)
{
	...
	while (j % params->nbClients != (end + 1) % params->nbClients)
	{
		...
		// Update servedBy Vector of offspring
		result->servedByVec[result->chromT[j % params->nbClients]->cour-1] = parents.first->servedByVec[result->chromT[j % params->nbClients]->cour-1];
		// Mark the client as copied
		freqClient[result->chromT[j % params->nbClients]->cour] = true;
		j++;
	}
	...
	for (int i = 1; i <= params->nbClients; i++)
	{
		...
		auto temp = parents.second->chromT[(end + i) % params->nbClients];
		// If the client is not yet copied in place, copy in place now
		if (freqClient[temp->cour] == false)
		{
			result->chromT[j % params->nbClients] = temp;
			// Update servedBy Vector of offspring
			result->servedByVec[result->chromT[j % params->nbClients]->cour-1] = parents.second->servedByVec[result->chromT[j % params->nbClients]->cour-1];
...
Individual* Genetic::crossoverSREX(std::pair<const Individual*, const Individual*> parents)
{
	...
	std::unordered_set<Node*> clientsInSelectedA;
	...
	std::unordered_set<Node*> clientsInSelectedB;
	for (int r = 0; r < nOfMovedRoutes; r++)
	{
	...
	while (improved)
	{
		...
		const int differenceALeft = static_cast<int>(std::count_if(parents.first->chromR[(startA - 1 + nOfRoutesA) % nOfRoutesA].begin(),
			...
			[&clientsInSelectedB](auto c) { return clientsInSelectedB.find(c) == clientsInSelectedB.end(); }))
		...
		const int differenceBLeft = static_cast<int>(std::count_if(parents.second->chromR[(startB - 1 + nOfMovedRoutes) % nOfRoutesB].begin(),
			...
			[&clientsInSelectedA](auto c) { return clientsInSelectedA.find(c) != clientsInSelectedA.end(); }))
		...
		if (bestDifference < 0)
		{
			...
			{
				for (auto c : parents.first->chromR[(startA + nOfMovedRoutes - 1) % nOfRoutesA])
				{
					clientsInSelectedA.erase(clientsInSelectedA.find(c));
				}
				startA = (startA - 1 + nOfRoutesA) % nOfRoutesA;
				for (auto c : parents.first->chromR[startA])
				{
			...
			{
				for (auto c : parents.second->chromR[(startB + nOfMovedRoutes - 1) % nOfRoutesB])
				{
					clientsInSelectedB.erase(clientsInSelectedB.find(c));
				}
				startB = (startB - 1 + nOfRoutesB) % nOfRoutesB;
				for (auto c : parents.second->chromR[startB])
				{
	...
	std::unordered_set<Node*> clientsInSelectedANotB;
	...
	std::unordered_set<Node*> clientsInSelectedBNotA;
	...
	for (int r = 0; r < nOfMovedRoutes; r++)
	{
		...
		for (auto c : parents.second->chromR[indexB])
		{
			candidateOffsprings[0]->chromR[indexA].push_back(c);
			// Update servedBy Vector of offspring
			candidateOffsprings[0]->servedByVec[c->cour-1] = parents.second->servedByVec[c->cour-1];
			if (clientsInSelectedBNotA.find(c) == clientsInSelectedBNotA.end())
			{
				candidateOffsprings[1]->chromR[indexA].push_back(c);
				// Update servedBy Vector of offspring
				candidateOffsprings[1]->servedByVec[c->cour-1] = parents.second->servedByVec[c->cour-1];
			}
		}
	}
	...
	for (int r = nOfMovedRoutes; r < nOfRoutesA; r++)
	{
		...
		for (auto c : parents.first->chromR[indexA])
		{
			if (clientsInSelectedBNotA.find(c) == clientsInSelectedBNotA.end())
			{
				candidateOffsprings[0]->chromR[indexA].push_back(c);
				// Update servedBy Vector of offspring
				candidateOffsprings[0]->servedByVec[c->cour-1] = parents.first->servedByVec[c->cour-1];
			}
			candidateOffsprings[1]->chromR[indexA].push_back(c);
			// Update servedBy Vector of offspring
			candidateOffsprings[1]->servedByVec[c->cour-1] = parents.first->servedByVec[c->cour-1];
		}
	}
...
void Genetic::insertUnplannedTasks(Individual* offspring, std::unordered_set<Node*> unplannedTasks)
{
	...
	for (auto c : unplannedTasks)
	{
		// Get the earliest and laster possible arrival at the client
		int earliestArrival = params->cli[c->servedBy].earliestArrival;
		int latestArrival = params->cli[c->servedBy].latestArrival;
		...
		for (int r = 0; r < params->nbVehicles; r++)
		{
			...
			newDistanceFromInsert = params->timeCost.get(c->servedBy, offspring->chromR[r][0]->servedBy);
			if (earliestArrival + newDistanceFromInsert < params->cli[offspring->chromR[r][0]->servedBy].latestArrival)
			{
				distanceDelta = params->timeCost.get(0, c->servedBy) + newDistanceToInsert
					- params->timeCost.get(0, offspring->chromR[r][0]->servedBy);
			...
			{
				newDistanceToInsert = params->timeCost.get(offspring->chromR[r][i - 1]->servedBy, c->servedBy);
				newDistanceFromInsert = params->timeCost.get(c->servedBy, offspring->chromR[r][i]->servedBy);
				if (params->cli[offspring->chromR[r][i - 1]->servedBy].earliestArrival + newDistanceToInsert < latestArrival
					&& earliestArrival + newDistanceFromInsert < params->cli[offspring->chromR[r][i]->servedBy].latestArrival)
				{
					distanceDelta = newDistanceToInsert + newDistanceFromInsert
						- params->timeCost.get(offspring->chromR[r][i - 1]->servedBy, offspring->chromR[r][i]->servedBy);
					if (distanceDelta < bestDistance)
					{
						bestDistance = distanceDelta;
						bestLocation = { r, i };
			...
			newDistanceToInsert = params->timeCost.get(offspring->chromR[r].back()->servedBy, c->servedBy);
			if (params->cli[offspring->chromR[r].back()->servedBy].earliestArrival + newDistanceToInsert < latestArrival)
			{
				distanceDelta = newDistanceToInsert + params->timeCost.get(c->servedBy, 0)
					- params->timeCost.get(offspring->chromR[r].back()->servedBy, 0);
				if (distanceDelta < bestDistance)
				{
					bestDistance = distanceDelta;
...
Individual* Genetic::bestOfSREXAndOXCrossovers(std::pair<const Individual*, const Individual*> parents)
{
	...
	if (params->rng() % 100 < (unsigned int) 100-(100*params->config.fractionSREX)){
		return offspringOX;
	}else{
