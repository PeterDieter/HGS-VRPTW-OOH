...
void Split::generalSplit(Individual* indiv, int nbMaxVehicles)
{
	...
	for (int i = 1; i <= params->nbClients; i++)
	{
		...
		cliSplit[i].demand = params->cli[indiv->chromT[i - 1]->cour].demand;
		cliSplit[i].serviceTime = params->cli[indiv->chromT[i - 1]->servedBy].serviceDuration;
		cliSplit[i].d0_x = params->timeCost.get(0, indiv->chromT[i - 1]->servedBy);
		cliSplit[i].dx_0 = params->timeCost.get(indiv->chromT[i - 1]->servedBy, 0);
		
		// The distance to the next client is INT_MIN for the last client
		if (i < params->nbClients)
		{
			cliSplit[i].dnext = params->timeCost.get(indiv->chromT[i - 1]->servedBy, indiv->chromT[i]->servedBy);
		}
		else
		{
	...
	if (params->isDurationConstraint)
	{
		...
		for (int i = 0; i < params->nbClients; i++)
		{
			...
			for (int j = i + 1; j <= params->nbClients && load <= 1.5 * params->vehicleCapacity ; j++)
			{
				...
				double cost = distance + cliSplit[j].dx_0 + params->penaltyVehicleCapacity * std::max(load - params->vehicleCapacity, 0);
			...
			for (int i = k; i < params->nbClients && potential[k][i] < 1.e29; i++)
			{
				...
				for (int j = i + 1; j <= params->nbClients && load <= 1.5 * params->vehicleCapacity ; j++) // Setting a maximum limit on load infeasibility to accelerate the algorithm
				{
					...
					double cost = distance + cliSplit[j].dx_0
						+ params->penaltyVehicleCapacity * std::max(load - params->vehicleCapacity, 0);
