...
Params::Params(const CommandLine& cl)
{
	...
	parcelPointCapacityPerCell = INT_MAX;
	parcelPointCapacity = INT_MAX;
	maxWalking = INT_MAX;
	costPerVehicle = INT_MAX;
	priceSensitivity = INT_MAX;
	...
	if (inputFile.is_open())
	{
		...
		else
		{
...

			{
				...
				if (content == "NUMBER_PARCEL_POINTS")
				{
					// Need to substract the depot from the number of nodes and the number of ParcelPoints
					inputFile >> content2 >> nbParcelPoints;
				}
				else if (content == "DIMENSION")
				{
					// Need to substract the depot from the number of nodes and the number of ParcelPoints
					inputFile >> content2 >> nbClients;
					nbClients = nbClients - 1 - nbParcelPoints;
				}
				...
				else if (content == "VEHICLE_CAPACITY")
				{
				...
				else if (content == "PARCELPOINT_CAPACITY_PER_CELL")
				{
					inputFile >> content2 >> parcelPointCapacityPerCell;
				}
				else if (content == "MAXIMAL_WALKING_DISTANCE")
				{
					inputFile >> content2 >> maxWalking;
				}
				else if (content == "COST_PER_VEHICLE")
				{
					inputFile >> content2 >> costPerVehicle;
				}
				else if (content == "PRICE_SENSITIVITY")
				{
					inputFile >> content2 >> priceSensitivity;
				}
				...
				else if (content == "EDGE_WEIGHT_SECTION")
				{
					...
					maxDist = 0;
					timeCost = Matrix(nbClients + 1 + nbParcelPoints);
					for (int i = 0; i <= nbClients + nbParcelPoints; i++)
					{
						for (int j = 0; j <= nbClients + nbParcelPoints; j++)
						{
				...
				else if (content == "NODE_COORD_SECTION")
				{
					...
					cli = std::vector<Client>(nbClients + 1 + nbParcelPoints);
					for (int i = 0; i <= nbClients + nbParcelPoints; i++)
					{
				...
				// Read the demand of each client (including the depot, which should have demand 0)
				else if (content == "PARCELPOINT_CAPACITY_NUMBER_CELLS")
				{
					cellsPerParcelPoint = std::vector<int>(nbParcelPoints);
					for (int i = 0; i < nbParcelPoints; i++)
					{
						int ppNr = 0;
						inputFile >> ppNr >> cellsPerParcelPoint[i];
					}
				}
	...
	parcelPointCapacity = parcelPointCapacityPerCell*5;
	...
	if (!isExplicitDistanceMatrix)
	{
		...
		maxDist = 0;
		timeCost = Matrix(nbClients + nbParcelPoints + 1);
		...
		for (int i = 0; i <= nbClients + nbParcelPoints; i++)
		{
			...
			for (int j = i + 1; j <= nbClients + nbParcelPoints; j++)
			{
	...
	orderProximities = std::vector<std::vector<std::pair<double, int>>>(nbClients + nbParcelPoints + 1);
	// Loop over all clients (excluding the depot)
	for (int i = 1; i <= nbClients + nbParcelPoints; i++)
	{
	...
	penaltyVehicleCapacity = std::max(0.1, std::min(1000., static_cast<double>(maxDist) / maxDemand));
	penaltyParcelCapacity = std::max(0.1, std::min(100000., static_cast<double>(maxDist) / maxDemand));
...
void Params::SetCorrelatedVertices(){
	...
	correlatedVertices = std::vector<std::vector<int>>(nbClients + nbParcelPoints + 1);

	// First create a set of correlated vertices for each vertex (where the depot is not taken into account)
	std::vector<std::set<int>> setCorrelatedVertices = std::vector<std::set<int>>(nbClients + nbParcelPoints + 1);
