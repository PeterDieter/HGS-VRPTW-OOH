...
void Population::doLocalSearchAndAddIndividual(Individual* indiv, bool applyParcelPointNeighborhood)
{
	// Do a Local Search
	localSearch->run(indiv, params->penaltyVehicleCapacity, params->penaltyTimeWarp, params->penaltyParcelCapacity, applyParcelPointNeighborhood);
	// Add an individual
	...
	{
		localSearch->run(indiv, params->penaltyVehicleCapacity * 10., params->penaltyTimeWarp * 10., params->penaltyParcelCapacity * 10., false);
...
void Population::generatePopulation()
{
	if (params->nbClients == 1)
	{
		...
		Individual randomIndiv(params, true, true);
		split->generalSplit(&randomIndiv, params->nbVehicles);
		localSearch->constructIndividualFromSplit(&indiv,randomIndiv);
		doLocalSearchAndAddIndividual(&indiv, true);
		return;
	}
	...
	for (int i = 0; i < nofNearestIndividualsToGenerate; i++)
	{
		...
		Individual indiv(params, false, true);
	...
	for (int i = 0; i < nofRandomIndividualsToGenerate; i++)
	{
		...
		Individual indiv(params, false, true);
		Individual randomIndiv(params, true, true, true);
		split->generalSplit(&randomIndiv, params->nbVehicles);
		localSearch->constructIndividualFromSplit(&indiv,randomIndiv);
		doLocalSearchAndAddIndividual(&indiv, true);
	}
...
bool Population::addIndividual(const Individual* indiv, bool updateFeasible)
{
	...
	if (updateFeasible)
	{
		listFeasibilityVehicleLoad.push_back(indiv->myCostSol.vehicleCapacityExcess < MY_EPSILON);
		listFeasibilityParcelPointLoad.push_back(indiv->myCostSol.parcelPointCapacityExcess < MY_EPSILON);
		listFeasibilityTimeWarp.push_back(indiv->myCostSol.timeWarp < MY_EPSILON);
		listFeasibilityVehicleLoad.pop_front();
		listFeasibilityParcelPointLoad.pop_front();
...
void Population::managePenalties()
{
	...
	double fractionFeasibleVehicleLoad = static_cast<double>(std::count(listFeasibilityVehicleLoad.begin(), listFeasibilityVehicleLoad.end(), true)) / static_cast<double>(listFeasibilityVehicleLoad.size());
	if (fractionFeasibleVehicleLoad <= 0.01 && params->config.penaltyBooster > 0. && params->penaltyVehicleCapacity < 100000.) {
		params->penaltyVehicleCapacity = std::min(params->penaltyVehicleCapacity * params->config.penaltyBooster, 100000.);
	}
	else if (fractionFeasibleVehicleLoad < params->config.targetFeasible - 0.05 && params->penaltyVehicleCapacity < 100000.)
	{
		params->penaltyVehicleCapacity = std::min(params->penaltyVehicleCapacity * 1.2, 100000.);
	}
	else if (fractionFeasibleVehicleLoad > params->config.targetFeasible + 0.05 && params->penaltyVehicleCapacity > 0.1)
	{
		params->penaltyVehicleCapacity = std::max(params->penaltyVehicleCapacity * 0.85, 0.1);
	}

	// Setting some bounds [0.1,100000] to the penalty values for safety
	double fractionFeasibleParcelPointLoad = static_cast<double>(std::count(listFeasibilityParcelPointLoad.begin(), listFeasibilityParcelPointLoad.end(), true)) / static_cast<double>(listFeasibilityParcelPointLoad.size());
	if (fractionFeasibleParcelPointLoad <= 0.01 && params->config.penaltyBooster > 0. && params->penaltyParcelCapacity < 100000.) {
		params->penaltyParcelCapacity = std::min(params->penaltyParcelCapacity * params->config.penaltyBooster, 100000.);
	}
	else if (fractionFeasibleParcelPointLoad < params->config.targetFeasible - 0.05 && params->penaltyParcelCapacity < 100000.)
	{
		params->penaltyParcelCapacity = std::min(params->penaltyParcelCapacity * 1.2, 100000.);
	}
	else if (fractionFeasibleParcelPointLoad > params->config.targetFeasible + 0.05 && params->penaltyParcelCapacity > 0.1)
	{
		params->penaltyParcelCapacity = std::max(params->penaltyParcelCapacity * 0.85, 0.1);
	}
	...
	for (int i = 0; i < static_cast<int>(infeasibleSubpopulation.size()); i++)
	{
		infeasibleSubpopulation[i]->myCostSol.penalizedCost = infeasibleSubpopulation[i]->myCostSol.distance
			+ infeasibleSubpopulation[i]->myCostSol.discountGiven
			+ params->penaltyVehicleCapacity * infeasibleSubpopulation[i]->myCostSol.vehicleCapacityExcess
			+ params->penaltyParcelCapacity * infeasibleSubpopulation[i]->myCostSol.parcelPointCapacityExcess
...
void Population::printState(int nbIter, int nbIterNoImprovement)
{
	...
	{
		std::printf(" | Inf %zu %d\n %.2f", infeasibleSubpopulation.size(), getBestInfeasible()->myCostSol.timeWarp, getAverageCost(infeasibleSubpopulation));
	}
	else
	{
	...
	std::printf(" | Feas %.2f %.2f", static_cast<double>(std::count(listFeasibilityVehicleLoad.begin(), listFeasibilityVehicleLoad.end(), true)) / static_cast<double>(listFeasibilityVehicleLoad.size()), static_cast<double>(std::count(listFeasibilityTimeWarp.begin(), listFeasibilityTimeWarp.end(), true)) / static_cast<double>(listFeasibilityTimeWarp.size()));
	std::printf(" | Pen %.2f %.2f", params->penaltyVehicleCapacity, params->penaltyTimeWarp);
	std::cout << std::endl;
}
...
void Population::logSolution(int nbIter, std::ofstream& myfile, Individual* indiv)
{
	...
	myfile << indiv->myCostSol.distance << ";" << indiv->myCostSol.discountGiven << ";" << indiv->myCostSol.vehicleCapacityExcess << ";" << indiv->myCostSol.timeWarp << ";" << indiv->myCostSol.parcelPointCapacityExcess << ";";
...
Population::Population(Params* params, Split* split, LocalSearch* localSearch) : params(params), split(split), localSearch(localSearch)
{
	...
	listFeasibilityVehicleLoad = std::list<bool>(100, true);
